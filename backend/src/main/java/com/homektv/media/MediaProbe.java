...
public record MediaProbe(
    ...
    public MediaProbe(long durationMs, int audioTracks, int subtitleTracks, boolean hasVideo, String resolution) {
        this(durationMs, audioTracks, subtitleTracks, hasVideo, resolution, List.of(), null, null);
    }

    /** 兼容旧调用：保留仅传音频流列表的构造器。 */
    public MediaProbe(long durationMs, int audioTracks, int subtitleTracks, boolean hasVideo, String resolution,
                      List<AudioStreamInfo> audioStreams) {
        this(durationMs, audioTracks, subtitleTracks, hasVideo, resolution, audioStreams, null, null);
    }
}
